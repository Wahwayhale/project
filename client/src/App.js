    ...
    <div className="app">
      <div className="sidebar">
        ...
        <div className="sidebar-tabs">
          ...
          <button className={`tab ${view === 'friends' ? 'active' : ''}`} onClick={() => setView('friends')}>
            👥 好友
            {friendRequests.length > 0 && <span className="badge">{friendRequests.length}</span>}
          </button>
          <button className={`tab ${view === 'video' ? 'active' : ''}`} onClick={() => setView('video')}>
            📺 视频
          </button>
          ...
          <div className="room-list">
            ...
            {friends?.map(friend => (
              ...
              >
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); showFriendPayCode(friend.username); }}
                  style={{
                    padding: '4px 8px',
                    fontSize: 11,
                    background: '#ff6b35',
            ...
            <div className="ai-header">
              <div>
                ...
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)', marginTop: 2 }}>
                  余额: ¥{balance.toFixed(2)} | ¥0.01/次
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button 
                  onClick={() => { setShowRechargeModal(true); fetchRechargeHistory(); }}
                  style={{ 
                    padding: '4px 10px', 
                    fontSize: 12, 
                    background: '#ff6b35', 
                ...
                {user?.username === 'admin' && (
                  <button 
                    onClick={() => { setShowAdminModal(true); fetchPendingRecharges(); }}
                    style={{ 
                      padding: '4px 10px', 
                      fontSize: 12, 
                      background: '#1890ff', 
                  ...
                  <div className="ai-bubble">
                    {msg.role === 'user' ? msg.content : (
                      <>
                        ...
                        {msg.rechargeUrl && (
                          <a
                            href={msg.rechargeUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="send-button"
                            style={{ marginTop: 8, display: 'inline-block', background: '#ff6b35', textDecoration: 'none' }}
                          >
                            💰 前往充值
                          </a>
                        )}
      ...
      <div className="main-chat">
        {currentRoom ? (
          ...
          <div className="empty-chat">
              ...
              <div className="empty-title">选择一个聊天开始</div>
              <div className="empty-desc">从左侧选择一个聊天室或添加好友开始对话</div>
            </div>
        )}
      </div>
