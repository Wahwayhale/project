...
WeChat App is a single-server Express + Socket.io backend serving a React SPA frontend, packaged as a Capacitor Android APK.

```
┌─ Browser ─────────────┐  ┌─ Android APK ────────────┐
│  React SPA (web)       │  │  React SPA (bundled)      │
│  axios → localhost:3001│  │  axios → ngrok → server   │
│  socket.io → :3001     │  │  socket.io → ngrok        │
└────────────────────────┘  └───────────────────────────┘
           │                          │
           ▼                          ▼
┌─ ngrok tunnel ──────────────────────────┐
...
| AI | Zhipu GLM-4V / Kimi / DeepSeek R1 (Tencent MAS) / Pollinations |
...
4. **Capacitor bundled** — APK loads local assets, API calls via ngrok; OTA update for web changes only
5. **ngrok tunneling** — Free public URL without server setup; acceptable for dev/personal use
